...
test("Clear command history purges persisted history", async ({ browser }) => {
    var result = await newTouchPage(browser);
    var page = result.page;
    await gotoReady(page);
    ...
    var before = await page.evaluate(() => JSON.parse(localStorage.getItem("we-term-history") || "[]"));
    expect(before.join("\n")).toContain("echo hello");

    await page.locator("#settings-btn").tap();
    await page.waitForTimeout(200);
    await page.locator("#clear-history-btn").tap();
    await page.waitForTimeout(200);

    var after = await page.evaluate(() => JSON.parse(localStorage.getItem("we-term-history") || "[]"));
